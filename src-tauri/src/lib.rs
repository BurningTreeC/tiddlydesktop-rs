...
/// Linux drag-drop handling
/// Using Tauri's native drag-drop handling to avoid interfering with WebKit's internal drag handling
#[cfg(target_os = "linux")]
mod linux_drag {
    use tauri::WebviewWindow;

    /// Set up drag-drop handling for a webview window
    /// Currently using native Tauri/WebKit handling for best compatibility with internal drags
    pub fn setup_drag_handlers(_window: &WebviewWindow) {
        // Let Tauri/WebKit handle drag-drop natively
        // This preserves WebKit's internal drag handling for text selections and links
    }
}

/// Windows drag-drop handling
/// Using native WebView2 drag-drop handling for best compatibility
#[cfg(target_os = "windows")]
mod windows_drag {
    use tauri::WebviewWindow;
    ...
    /// Set up drag-drop handling for a webview window
    /// Enables native WebView2 drag-drop handling for best compatibility
    pub fn setup_drag_handlers(window: &WebviewWindow) {
        ...
        let _ = window.with_webview(move |webview| {
            ...
            unsafe {
                ...
                // Enable WebView2's native external drop handling
                // This lets the webview handle drag-drop natively without our custom OLE handlers
                ...
                // No custom OLE drop target registration - let WebView2 handle everything natively
                eprintln!("[TiddlyDesktop] Windows: Using native WebView2 drag-drop handling");
            }
        });
    }
}
...
/// Using native WebKit drag-drop handling for best compatibility
#[cfg(target_os = "macos")]
mod macos_drag {
    use tauri::WebviewWindow;

    /// Set up drag-drop handling for a webview window
    /// Currently using native WebKit handling for best compatibility with internal drags
    pub fn setup_drag_handlers(_window: &WebviewWindow) {
        // Let WebKit handle drag-drop natively
        // This preserves WebKit's internal drag handling for text selections and links
    }
}
